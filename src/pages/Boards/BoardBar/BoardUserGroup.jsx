...
function BoardUserGroup({ boardUsers = [], limit = 8 }) {
  ...
  // Lưu ý ở đây chúng ta không dùng Component AvatarGroup của MUI bởi nó không hỗ trợ tốt trong việc chúng ta cần custom & trigger
  // xử lý phần tử tính toán cuối, đơn giản là cứ dùng Box và CSS - Style đám Avatar cho chuẩn kết hợp tính toán một chút thôi.
  return (
    <Box sx={{ display: 'flex', gap: '4px' }}>
      {/* Hiển thị giới hạn số lượng user theo số limit */}
      {[...Array(16)].map((_, index) => { // Tao mang co 16 phan tu la undefined
        if (index < limit) {
          return (
            <Tooltip title="minhnang" key={index}>
              <Avatar
                sx={{ width: 34, height: 34, cursor: 'pointer' }}
                alt="minhnang"
                src="https://th.bing.com/th/id/OIP.PBW5xYA2INIu23KqsVN1RAAAAA?pid=ImgDet&w=184&h=230&c=7&dpr=1.3"
              />
            </Tooltip>
          )
        }
